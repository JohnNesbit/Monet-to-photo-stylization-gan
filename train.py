...
r.urlretrieve("https://people.eecs.berkeley.edu/~taesung_park/CycleGAN/datasets/monet2photo", "monet2photo")
os.rename("monet2photo", "monet2photo.zip")


with zipfile.ZipFile("monet2photo.zip", 'r') as zip_ref:
    zip_ref.extractall("monet2photo")


b_path = "monet2photo/monet2photo/" # path to dir containing test and train folders
...
for m in range(2):
    ...
    for lol in range(2):
        ...
        for i in os.listdir(path):
            rawi = misc.imread(path + "/" + i)
            bi, gi, ri = np.reshape(rawi, [3, 256, 256])
            im = np.array([ri, gi, bi])
            print(im.shape)
            exec(t_t[m] + b_s[lol] +" = np.append(" + t_t[m] + b_s[lol] + ", im)")
...
a_train = np.reshape(trainA, newshape=[batch_size, 256, 256, 3])
b_train = np.reshape(trainB, newshape=[batch_size, 256, 256, 3])
b_test = np.reshape(testB, newshape=[test_size, 256, 256, 3])
a_test = np.reshape(testA, newshape=[test_size, 256, 256, 3])
...
def train():
	for i in range(epochs):
		for x in range(batch_size):
			...
			sess.run([d_A_trainer, a_descrim_loss],
			{a_placeholder: a_real_image_batch,b_placeholder: b_real_image_batch})
		...
		saver.save(sess, "models/pretrained_mon" + str(i) +".ckpt", global_step=i)
		print("EPOCH:", i)
